"""Replication engine for the clinic queue simulation.

This module runs one independent simulation replication for a supplied
...
def process_patient_route(
    ...
    """Process one patient through all route stations and record metrics."""
    for station_name in ROUTES[patient.route_id]:
        ...
        with resource.request() as request:
            ...
            if station_name in (
                ...
                metrics.record_station_waiting(
                    station_name,
                    waiting_time,
                )
...
def generate_patient_arrivals(
    ...
    """Generate patients using random arrival intervals."""
...
def run_replication(scenario: dict[str, int], seed: int) -> SimulationMetrics:
    """Run one independent replication and return its collected metrics."""
    random.seed(seed)
    np.random.seed(seed)
